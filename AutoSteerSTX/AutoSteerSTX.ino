...
//steering variables
int sensors = 2;   // set here for 4WS or Case STX accusteer = 2, 2WS or Artic = 1
float steerAngleActual = 0;
int steerPrevSign = 0, steerCurrentSign = 0; // the steering wheels angle currently and previous one
float steerAngleSetPoint = 0; //the desired angle from AgOpen
int steeringPosition = 0, steeringPositionZero = 544; //from steering sensor
int steeringPosition2 = 0;  // 2nd steerpos sensor
int steer2 = 0; // corrected steer position
float steerAngleError = 0; //setpoint - actual
float distanceError = 0; //
float steerSensorCounts = 13;
...
void loop()
{
    ...
    steeringPosition = analogRead(A0);
    steeringPosition2 = analogRead(A2);     delay(2);
    steeringPosition += analogRead(A0);
    steeringPosition2 += analogRead(A2);     delay(2);
    steeringPosition += analogRead(A0);
    steeringPosition2 += analogRead(A2);     delay(2);
    ...
    steer2 = map(steeringPosition2,313,773,433,653); // map front  sensor to match sensorposition counts/degrees
    
    //steeringPosition = ( steeringPosition - steeringPositionZero + XeRoll/16.0);   //read the steering position sensor
    //steeringPosition2 = ( steeringPosition2 - steeringPositionZero2 + XeRoll/16.0);   //read the steering position sensor
    steeringPosition = ( steeringPosition - steeringPositionZero);   //read the steering position sensor
    steer2 = (steer2 - steeringPositionZero);   //read the steering position sensor
    
    //convert position to steer angle. 6 counts per degree of steer pot position in my case
    //  ***** make sure that negative steer angle makes a left turn and positive value is a right turn *****
    // remove or add the minus for steerSensorCounts to do that. 
    steerAngleActual = (float)(steeringPosition/-steerSensorCounts)+(steer2/steerSensorCounts);  
    }
